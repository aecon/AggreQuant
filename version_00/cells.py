...
def segment_propagation(image_file, seeds_file, allnuclei_file, opath, Names):
...
def cellbody_segmentation(cellbody_images, Names):
    ...
    for ifile, image_file in enumerate(cellbody_images):
        ...
        # Output folder for cellbody segmentation
        opath = "%s/%s/%s" % ( Names.OUTDIR_PATH, Names.OUTDIR, Names.CELLBODY_ODIR_NAME)
        ...
        if segmentation_algorithm == "distance":
            ...
            segment_propagation(image_file, seeds_file, allnuclei_file, opath, Names)
